    ...
    try {
        ...
        switch (jVarLocalParse?.Type) {
            ...
            case "ChatACK":
                ...
                break;
            case "WaSendFail":
                jFLocalForWaSendFail({ inData: jVarLocalParse.ChatLog });
...
const jFLocalForWaSendFail = ({ inData }) => {
    if (inData === "time") {
        jFLocalDisplayShowMessageFailedId();
        jFLocalDisplayNoneMessageSentId();
    };
};

const jFLocalForChatACK = ({ inData }) => {
    if (inData === undefined === false) {
        // console.log("WASend : ", inData);
        // debugger;
        if (inData?.Type === "WASend") {
            jFLocalDisplayNoneMessageFailedId();
...
let jFLocalDisplayShowMessageFailedId = () => {
    let jVarLocalHtmlId = 'MessageFailedId';
    let jVarLocalMessageFailedId = document.getElementById(jVarLocalHtmlId);

    if (jVarLocalMessageFailedId === null === false) {
        jVarLocalMessageFailedId.style.display = '';
    };
};

let jFLocalDisplayNoneMessageFailedId = () => {
    let jVarLocalHtmlId = 'MessageFailedId';
    let jVarLocalMessageFailedId = document.getElementById(jVarLocalHtmlId);

    if (jVarLocalMessageFailedId === null === false) {
        jVarLocalMessageFailedId.style.display = 'none';
    };
};
...
let jFLocalDisplayNoneMessageSentId = () => {
    let jVarLocalHtmlId = 'MessageSentId';
    let jVarLocalMessageSentId = document.getElementById(jVarLocalHtmlId);

    if (jVarLocalMessageSentId === null === false) {
        jVarLocalMessageSentId.style.display = 'none';
    };
};
